...
def store_news_to_db(news_content_result, db_manager, source_url, logger):
    for news in news_content_result:
        ...
        file_path = f"content/{source_url.split('//')[1].split('.')[1]}/{timestamp}_{title[:30]}.txt"
        ...
        try:
            db_manager.execute_sql(INSERT_NEWS_ARTICLE, news_data)
...
def main():
    ...
    while True:
        try:
            ...
            store_news_to_db(reuters_news, db_manager, "https://www.reuters.com/")
            
            #bbc_news = get_news_source_with_scrapegraphai("https://www.bbc.com/news", get_LLM_AI_key(args.llm_model))
            #store_news_to_db(bbc_news['news'], db_manager, "https://www.bbc.com/")
