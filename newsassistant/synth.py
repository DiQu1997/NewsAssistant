...
- **有程序节点的事优先给一张 stepper**：谈判轮次、立法进程、计划分步、
  停火分阶段——只要事件是「沿着既定节点往前走」，就值得一张 stepper。
- **不要因为进度早而跳过它**：即使现在只推进到第一步、后面全是 pending，
  「计划仍停在第一步、投票与落地都未发生」本身就是发现，画出来比不画有用。
  已完成、进行中、待定的节点对比，正是读者想知道的。"""
