...
| 中文 | English | 在本项目里的意思 |
|---|---|---|
| **结构先验** | structural prior | 对任何领域都成立的假设：文档里有宣称、宣称有信源、实体有身份、事件有时间、信源有层级。**这些硬编码**，是系统地基（D2） |
| **主题先验** | topical prior | "半导体有六个环节"、"这些公司重要"这类领域知识。**一个都不硬编码**，全部由数据涌现。代码路径里出现领域词汇即违规 |
| **涌现** | emergence | 系统没被告知某个主题，却因为数据里的结构（实体反复共现、宣称反复指向同一事件）而自己长出对它的表示 |
...
| **宣称** | claim · `claims` | 从文档里抽出的**一句可验证的陈述** | **系统的差异化核心**。综述的每句话都必须引用它；分歧、立场、引用全部建立在它上面 |
...
| `stance` | stance | −2…+2，**本文对该宣称的立场**，不是事实判断。分歧矩阵的燃料 |
...
| **分歧矩阵** | disagreement matrix | 同一宣称多源立场不一。**通用，核心资产** —— 参照项目做不出来，因为它们的数据没有 claim 层 |
